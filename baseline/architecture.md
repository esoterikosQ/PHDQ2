...
| `batch_size` | 64 | `BART_PROFILE=paper`; `tuned`는 32 |
| `lr` | 3e-05 | `BART_PROFILE=paper`; `tuned`는 5e-05 |
| `max_epochs` | 10 | `BART_PROFILE=paper`; `tuned`는 40 |
| `warmup_ratio` | 0.0 | |
| `weight_decay` | 0.01 | bias, LayerNorm 제외 |
| `num_beams` | 4 | 추론 시 beam search |

### 1-3. 옵티마이저/스케줄러
- **Optimizer**: AdamW (weight_decay=0.01, bias/LayerNorm 제외, 기본 `correct_bias=False`)
- **Scheduler**: Linear warmup → linear decay
- `num_train_steps = data_len * max_epochs / batch_size`
- **Run profile**: `scripts/train_bart.sh`는 기본 `BART_PROFILE=paper`로 논문 재현 설정을 사용하고, 기존 탐색 설정은 `BART_PROFILE=tuned`로 분리한다.
- **M2**: `M2_SOURCE_GOLD_PATH`가 제공될 때만 m2scorer를 실행한다. 미제공 시 M2는 unavailable로 기록한다.
