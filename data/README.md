...
**git에 커밋되지 않도록 `.gitignore`에서 `data/` 전체를 제외합니다.** (대용량 파일 방지)

현재 로컬 워크스페이스에는 다음 계열의 데이터/산출물이 있을 수 있습니다.

- `Raw/`: 원본 코퍼스
- `Preprocessed/`: 학습/평가용 전처리 결과
- `generated_outputs/`: 실험 run 번호와 연결된 생성 결과
